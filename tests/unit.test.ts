...
// ── 한국어 시각 라벨 파싱 ──────────────────────────────────
import { minutesToHHMM, parseTimeLabels } from '../src/timelabel.js';

const asHHMM = (labels: string[]) => parseTimeLabels(labels).map((m) => (m === undefined ? '?' : minutesToHHMM(m)));

test('만나교회 시간표처럼 오전·오후 표시 없이 이어지는 목록을 읽는다', () => {
  const labels = ['9시', '9시30분', '10시', '10시30분', '11시', '11시30분', '12시', '12시30분',
                  '1시', '1시30분', '2시', '8시', '8시30분', '9시', '9시30분'];
  assert.deepEqual(asHHMM(labels), [
    '09:00', '09:30', '10:00', '10:30', '11:00', '11:30', '12:00', '12:30',
    '13:00', '13:30', '14:00', '20:00', '20:30', '21:00', '21:30',
  ]);
});

test('오전·오후가 적혀 있으면 그대로 따른다', () => {
  assert.deepEqual(asHHMM(['오전 9시', '오후 1시', '오후 9시30분']), ['09:00', '13:00', '21:30']);
});

test('콜론 표기와 24시간제도 읽는다', () => {
  assert.deepEqual(asHHMM(['09:00', '13:30', '21:00']), ['09:00', '13:30', '21:00']);
});

test('오후부터 시작하는 목록도 이른 새벽으로 읽지 않는다', () => {
  assert.deepEqual(asHHMM(['1시', '1시30분', '2시']), ['13:00', '13:30', '14:00']);
});
