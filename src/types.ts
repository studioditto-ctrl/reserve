...
export interface JobConfig {
  ...
  watch?: {
    ...
    maxWaitMin?: number;
    /**
     * 사이트가 오픈 시각에 며칠 뒤 날짜를 여는지. 만나교회는 8 (토요일 밤 → 다음주 일요일).
     *
     * 지정하면 아직 열리지 않은 먼 날짜를 빼고, **이미 열린 날 중 가장 먼 날**부터
     * 확인합니다. 오픈 순간에 막 열린 날이 바로 그 날이기 때문입니다.
     * 없으면 예전처럼 가까운 날부터 봅니다.
     */
    leadDays?: number;
    /**
     * 한 회차에서 확인할 날짜 수 상한.
     * leadDays 가 있으면 "이미 열린 가장 먼 날" 부터, 없으면 가까운 날짜부터 셉니다.
     * 오픈 순간에 엉뚱한 날짜까지 훑느라 시간을 쓰지 않도록 1 로 두면 좋습니다.
     */
    maxDates?: number;
  };
}
